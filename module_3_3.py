...
#print_params(1, "string", False, 8, [1, 2, 3]) - вызов функции с разным количеством аргументов не возможен, т.к. изначально в функции задано только три аргумента
...
print_params(b = 25) # - работает, т.к. произведено переназначение параметра "b"
print_params(c = [1,2,3]) # - аналогично предыдущему примеру. Значение параметра "с" было зпменено на список
...
print_params(*values_list_2, 42) #- вызов функции работает. Из списка распаковалось два параметра, плюс мы добавили третий "42", что соответствует количеству параметров в функции